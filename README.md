...
- 화면 구성 변경<br />
  ① Story.js: 동물 나라 이야기를 전달하는 스토리 진행 화면 추가.<br />
  ② ResultTransitionScreen.js: 질문 화면과 결과 화면 사이에 전환 페이지를 도입하여 서사적 연결성 강화.<br />
  ③ ResultScreen.js: 사용자의 선택에 따라 적합한 역사적 인물을 추천.<br />
...
## 20250122 결과 화면 저장 기능 추가 (윤지)
- **추가된 기능**: 테스트 결과 화면을 캡처하여 사용자의 로컬 디바이스에 저장할 수 있는 기능을 구현했습니다.
- **사용 라이브러리**:
  - **`html-to-image`**: 초기 구현 시 사용했으나 모바일 환경에서 일부 이미지가 비정상적으로 렌더링되는 문제가 발생했습니다.
  - **`html2canvas`**: 최종적으로 모바일 및 데스크톱 환경에서 안정적으로 동작하여 해당 라이브러리로 변경하였습니다.
🔧 구현 과정
1. **`html-to-image` 라이브러리**
   - 화면 캡처 및 저장을 위해 처음 사용한 라이브러리.
   - 문제점:
     - 모바일 환경에서 이미지가 하얗게 출력되거나 일부 요소가 렌더링되지 않는 문제가 발생.
     - 특히 CORS(교차 출처 리소스 공유) 문제로 이미지가 캡처되지 않는 사례가 다수 발생.

2. **`html2canvas` 라이브러리**
   - 대체로 선택한 라이브러리.
   - 이유:
     - 모바일과 데스크톱 모두에서 안정적으로 화면을 캡처.
     - CORS 문제를 `useCORS: true` 옵션으로 해결 가능.
